...
public interface EstudianteRepository extends JpaRepository<Estudiante, Long> {
    @Query("SELECT NEW com.prueba.examen.dto.EstudianteDTO(e.id, e.zonaHoraria) FROM Estudiante e  WHERE e.id = :idEstudiante")
   EstudianteDTO idEstudiante(@Param("idEstudiante") Long idEstudiante);

    @Query("SELECT NEW com.prueba.examen.dto.EstudianteDTO(e.id, e.nombre, e.edad, e.ciudad, e.zonaHoraria) FROM Estudiante e  WHERE e.id = :idEstudiante")
